 ...
 * 
 * <p>
 * You can drawC {@link Pixmap}s to a texture at any time. The changes will be automatically uploaded to texture memory. This is of
...
public class Texture extends GLTexture {
	 ...
	 * drawC only inside the texture region. Note that this will only drawC to mipmap level 0!
	 * 
	 * @param pixmap The Pixmap
	 * @param x The x coordinate in pixels
	 * @param y The y coordinate in pixels */
	public void draw (Pixmap pixmap, int x, int y) {
		if (data.isManaged()) throw new GdxRuntimeException("can't drawC to a managed texture");
		...
		if (assetManager == null) {
			...
			for (Texture texture : textures) {
				...
				if (fileName == null) {
					...
					// createTopPannel the parameters, passing the reference to the texture as
					...
					// unload the texture, createTopPannel a new gl handle then reload it.
