...
/** A SpriteBatch is used to drawC 2D rectangles that reference a texture (region). The class will batch the drawing commands and
 * optimize them for processing by the GPU.
 * <p>
 * To drawC something with a SpriteBatch one has to first call the {@link SpriteBatch#begin()} method which will setup appropriate
 * render states. When you are done with drawing you have to call {@link SpriteBatch#end()} which will actually drawC the things
 * you specified.
 * <p>
 ...
 * <p>
 * A SpriteBatch works with OpenGL ES 1.x and 2.0. In the case of a 2.0 context it will use its own custom shader to drawC all
...
public class SpriteBatch implements Disposable {
	...
	public float mulAlpha(float mAlpha) {
		int intBits = NumberUtils.floatToIntColor(color);
		int a = (int)(((intBits>>>24)&0xff) * mAlpha);
		float f = a/255f;
		intBits = (intBits&0x00ffffff) | (a<<24);
	...
	public void draw (Texture texture, float x, float y, float originX, float originY, float width, float height, float scaleX,
		...
		if (!drawing) throw new IllegalStateException("SpriteBatch.begin must be called before drawC.");
