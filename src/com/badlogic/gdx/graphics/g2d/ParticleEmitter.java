...
public class ParticleEmitter {
	...
	public ParticleEmitter (ParticleEmitter emitter) {
		...
		this.accumulator = emitter.accumulator;
		if(emitter.sprite != null){
			sprite = new Sprite(emitter.sprite);
		}
		{// Particle
			this.particles = new Particle[emitter.particles.length];
			for(int i=0; i<particles.length; i++){
				if(emitter.particles[i]!=null){
					this.particles[i] = new Particle(emitter.particles[i]);
				}
			}
		}
		...
		{// active array;
			if(emitter.active!=null){
				active = new boolean[emitter.active.length];
				for(int i=0; i<active.length; i++){
					this.active[i] = emitter.active[i];
				}
			}
		}
		this.x = emitter.x;
...
//              setMaxParticleCount(emitter.maxParticleCount);
		this.firstUpdate = emitter.firstUpdate;
		this.flipX = emitter.flipX;
		this.flipY = emitter.flipY;
		this.updateFlags = emitter.updateFlags;
		this.allowCompletion = emitter.allowCompletion;
		...
		this.spawnWidth = emitter.spawnWidth;
		this.spawnWidthDiff = emitter.spawnWidthDiff;
		this.spawnHeight = emitter.spawnHeight;
		this.spawnHeightDiff = emitter.spawnHeightDiff;
	...
	static int readInt (BufferedReader reader, String name) throws IOException {
		String v = readString(reader, name);
