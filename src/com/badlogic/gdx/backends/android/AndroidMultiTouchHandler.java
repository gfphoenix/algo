	...
	public void onTouch (MotionEvent event, AndroidInput input) {
		...
		synchronized (input) {
			...
			case MotionEvent.ACTION_POINTER_DOWN:
				...
				if(realPointerIndex >= input.realId.length || realPointerIndex >= input.touchX.length) return;
...
//				break;
			...
			case MotionEvent.ACTION_CANCEL:
				...
				int size = event.getPointerCount() < input.realId.length ? event.getPointerCount() : input.realId.length;
				for(int i = 0; i < size; i++){
					pointerId = event.getPointerId(i);
					realPointerIndex = input.lookUpPointerIndex(pointerId);
					if(realPointerIndex < 0 || realPointerIndex >= input.realId.length) continue;
					if(input.realId[realPointerIndex] == -1) continue;
			...
			case MotionEvent.ACTION_MOVE:
				...
				for (int i = 0; i < pointerCount; i++) {
					...
					if(realPointerIndex >= input.realId.length || realPointerIndex >= input.touchX.length) continue;
