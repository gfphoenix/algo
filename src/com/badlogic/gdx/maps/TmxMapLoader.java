...
public class TmxMapLoader {
	...
	/** Loads the  TiledMap from the given file. The file is resolved via the {@link FileHandleResolver} set in the
	 * constructor of this class. By default it will resolve to an internal file.
	 * @param tmxFile the filename
	 * @return the TiledMap */
	public TiledMapData load (FileHandle tmxFile) {
		try {
...
//			TiledMapData map = loadTilemap(root, tmxFile, imageResolver);
//			map.setOwnedResources(textures.values().toArray());
			return map;
		} catch (IOException e) {
			throw new GdxRuntimeException("Couldn't load tilemap '" + tmxFile.name() + "'", e);
		}
	}

	protected TiledMapData loadTilemap (Element root, FileHandle tmxFile) {
...
//		}
	 ...
	 * */
	protected void loadTileSet (TiledMapData map, Element element, FileHandle tmxFile) {
...
//		}
	 ...
	 * @param map
	 * @param element */
	protected void loadTileLayer (TiledMapData map, Element element) {
		if (element.getName().equals("layer")) {
			...
			int []layer_ids = new int[width * height];
			int k=0;
			layer.ids = layer_ids;
	...
	protected void loadObjectGroup (TiledMapData map, Element element) {
		if (element.getName().equals("objectgroup")) {
			String name = element.getAttribute("name", null);
			ObjectGroup layer = new ObjectGroup();
			layer.setName(name);
			Element properties = element.getChildByName("properties");
			if (properties != null) {
				loadProperties(layer.getProperties(), properties);
	...
	protected void loadObject (ObjectGroup layer, Element element) {
