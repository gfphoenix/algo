...
public class Actor implements Disposable {
	...
	float rotation;
	private int tag; // used by user to identify or class actors
	final Color color = new Color(1, 1, 1, 1);

	/** Draws the actor. The SpriteBatch is configured to drawC in the parent's coordinate system.
	 * {@link SpriteBatch#draw(com.badlogic.gdx.graphics.g2d.TextureRegion, float, float, float, float, float, float, float, float, float)
	 * This drawC method} is convenient to drawC a rotated and scaled TextureRegion. {@link SpriteBatch#begin()} has already been
	 * called on the SpriteBatch. If {@link SpriteBatch#end()} is called to drawC without the SpriteBatch then
