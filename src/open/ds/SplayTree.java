 ...
 */
public class SplayTree<T> implements Iterable<T> {
     ...
     * @return
     */
    public static <T> SplayTree<T> create(Comparator<T> comparator) {
        return new SplayTree<T>(comparator);
    }
     ...
     * @return
     */
    public static <T> SplayTree<T> create(Comparator<T> comparator, T... elements) {
        SplayTree<T> tree = new SplayTree<T>(comparator);
    ...
    public boolean insert(T element) {
        ...
        final int c = comparator.compare(element, root.key);
        if (c == 0) {
            return false;
        }
    ...
    public boolean remove(T element) {
        ...
        if (comparator.compare(element, root.key) != 0) {
            return false;
        }
    ...
    public T find(T element) {
        if (root == null) return null;
        splay(element);
        if (comparator.compare(element, root.key) != 0) return null;
        return root.key;
    }
    ...
    private void splay(T element) {
        ...
        while (true) {
            final int comp = comparator.compare(element, t.key);
            if (comp < 0) {
                if (t.left == null) break;
                if (comparator.compare(element, t.left.key) < 0) {
                ...
                if (comparator.compare(element, t.right.key) > 0) {
